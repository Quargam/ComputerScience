...
int my_ctime(char *out_buf, size_t buf_len,
             const struct timespec *file_timestamp) {
    ...
    return snprintf(out_buf, buf_len, "%s.%09ld %s", yyyymmdd_hhmmss,
                    file_timestamp->tv_nsec, tz_str);
}
