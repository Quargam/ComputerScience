...
ssize_t pwriteall(int fd, const void *buf, size_t count, long int offset) {
    ...
    while (bytes_written < count) {
        ssize_t res =
            pwrite(fd, buf_addr + bytes_written, count - bytes_written, offset);
        if (res < 0)
            return res;
...
size_t copying(const int fd1, const int fd2) {
    ...
    while (tst) {
        void *buf =
            calloc(BLCSZ, sizeof(char)); // allocation 4Kb (blocksize) of memory
...
int main(int argc, char *argv[]) {
    ...
    default:
        fprintf(stderr,
                "File %s is not type of 'Block device', 'Character device', "
                "'Regular', 'FIFO' or 'Symlink'\n",
                argv[1]);
        return 5;
    }
    return 0;
